...
test(
	...
	async ({apiHelpers, fdsSamplePage, page, site}) => {
		...
		await test.step(
			'Create a second page and reload to make it appear in the menu bar so we can navigate away',
			async () => {
		...
		await test.step(
			...
			async () => {
				const firstPageMenuItem = page.getByRole('menuitem', {
					name: fdsPageTitle,
				}).first();
...
test(
	...
	async ({page, pageEditorPage}) => {
		...
		await test.step('Drop a second FDS Sample widget on the page', async () => {
			...
			await pageEditorPage.addWidget('Sample', 'Frontend Data Set Sample');
		...
		await test.step(
			...
			async () => {
				...
				await selectFDSTab({fdsWidget: secondFDS, label: 'Content Renderers'});
		...
		await test.step(
			'Searches in the second (Content Renderers) FDS do not reach the Custom Element',
			async () => {
...
test(
	...
	async ({fdsSamplePage, page, pageEditorPage}) => {
		...
		await test.step(
			'Add a second Custom Element 7 widget to the page',
			async () => {
				await page.goto(`${fdsPageUrl}?p_l_mode=edit`);
